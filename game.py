from numba import njit, boolean, double
...
class Game(object):
    ...
    def play(self, move: Move) -> 'Game':
        ...
        if move.move_type == ATTACK:
            ...
            new_state[target_hand_idx] = min(
                new_state[target_hand_idx] + new_state[source_hand_idx], 5
            ) % 5

        else:
            # Handle SWAP move
            source_hand_idx = self.source_idxs[0 if move.source_hand == LEFT_HAND else 1]
            target_hand_idx = self.source_idxs[0 if move.source_hand == LEFT_HAND else 1]
            ...
            assert new_state[target_hand_idx] <= 4, "Target hand cannot exceed 4 fingers"

        hs = hash(self)
        new_seen_states = self.seen_states.copy()
        new_seen_states[hs] = 0.
    ...
    def get_state_value(self):
        ...
        if self.is_terminal():
            ...
            if p1_dead and not p2_dead:
                return -1. if self.player_1_turn else 1.
...
def get_initial_cache():
    return Cache(np.zeros(4 * 5**4 + 4 * 5**3 + 4 * 5**2 + 4 * 5 + 2, dtype=np.float64) + np.nan)